...
int binaria (Tree * raiz){
    if (raiz==NULL){
        return 1;
    }else if ((raiz->sad==NULL && raiz->sad!=NULL)||(raiz->sad!=NULL && raiz->sad==NULL)){
        return 0;
    }
    else if (raiz->sad!=NULL && raiz->sad!=NULL){
        int esq=binaria(raiz->sae);
        int dir=binaria(raiz->sad);
        if (esq!=0&&dir!=0){
            return 1;
        }
...
void insereInvertido (Tree ** t, int num){
    ...
    else
    {
        ...
        {
            /* Percorre pela subárvore à esquerda */
            insereInvertido(&(*t)->sae, num);
        }
        if(num < (*t)->num) /* Se o número for maior então vai pra direita */
        {
            /* Percorre pela subárvore à direita */
            insereInvertido(&(*t)->sad, num);
        }
    }
}
void caminhamentoInvertido(Tree* t)
{
    /* Essa função imprime os elementos de forma recursiva */
...
//  printf("<"); /* notação para organizar na hora de mostrar os elementos */
    ...
    {
        /* Mostra os elementos com caminhamento central*/
        caminhamentoInvertido(t->sad); /* mostra a sae (subárvore à esquerda) */
        printf("%d ", t->num); /* mostra a raiz */
        caminhamentoInvertido(t->sae); /* mostra a sad (subárvore à direita) */
    }
//  printf(">"); /* notação para organizar na hora de mostrar os elementos */
}
...
//    printf("\n%d\n",bi);
    Tree* t1 = createTree();
    printf("\n\n\n");
    insereInvertido(&t1, 50);/*Insere o 50 na árvore*/
    insereInvertido(&t1, 70);
    insereInvertido(&t1, 80);
    insereInvertido(&t1, 75);
    insereInvertido(&t1, 40);
    insereInvertido(&t1, 45);
    insereInvertido(&t1, 30);
    insereInvertido(&t1, 20);

    caminhamentoInvertido(t1);

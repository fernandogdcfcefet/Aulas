...
/** FUNCAO CRIADA PARA A QUESTAO */

void criaArvore(Tree *t, Tree **saida){

    if(!treeIsEmpty(t))
    {

        criaArvore(t->sae,saida);
        if(t->sae != NULL && t->sad != NULL){
            insertTree(saida,t->num);
        }
        criaArvore(t->sad,saida);
    }
}
...
int main()
{
    ...
    /** USANDO FUNCAO CRIADA PARA A QUESTAO */

    criaArvore(t,&saida);
    caminhamentoCentral(saida);

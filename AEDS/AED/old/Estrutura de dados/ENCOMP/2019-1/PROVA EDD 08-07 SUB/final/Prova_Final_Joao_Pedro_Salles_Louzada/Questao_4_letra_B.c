...
Tree** espelhaArvore(Tree **raiz) {

    if(!treeIsEmpty(*raiz))        /** Condição de parada, que interrompe a recursão*/
    {
        Tree** aux = createTree();
        aux = raiz;                     /** Efetua a troca do apontador para a direita com o apontador para a esquerda*/
        (*raiz)->sad = (*raiz)->sae;
        (*raiz)->sae = aux;

        espelhaArvore( &((*raiz)->sad) );   /** Chamada recursiva*/
        espelhaArvore( &((*raiz)->sae) );
    }

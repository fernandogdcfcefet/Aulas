...
/** As seguintes funções encontram o tamanho da arvore, guarda este numero em uma variavel, e cria um vetor deste tamanho.
    Em seguida percorre a arvore usando o caminhamento central guardando os valores no vetor */

void guardaTamanhoArvore(Tree* t, int* contador, int* vet_arv[MAXTAM]) { /** contador deve ser inicializado no main como 0 */

    if(!treeIsEmpty(t)) {
        guardaTamanhoArvore(t->sae, &contador, &vet_arv);
        vet_arv[(int) contador] = t->num;
        contador++;
        guardaTamanhoArvore(t->sad, &contador, &vet_arv);
    }

}

int encontraValorMediano(int* tam_arv, int* contador) { /** Contador já vem inicializado do main com o tamanho do vetor, enconrado na função acima*/

    if((int) contador%2 == 0) { /** Encontra mediano quando numero de elementos é par */
        return tam_arv[(int) contador/2];
    }
    else {  /** Encontra mediano quando numero de elementos é ímpar */ // else if (contador%2 == 1)
        return tam_arv[(int) contador/2];
    }

}

/** Por fim, a inserção é feita usando a função "void insertTree(Tree** t, int num)", passando o retorno da função encontraValorMediano como o num do parâmetro. */
...
int main()
{
    ...
    guardaTamanhoArvore(t, &contador, vet_arv);
    x = encontraValorMediano(vet_arv, &contador);

    for(i = 0; i < 11; i++){
        insertTree(&t2, vet_arv[i]);
    }
    puts("\n");
    caminhamentoCentral(&t2);

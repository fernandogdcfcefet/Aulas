...
int procura(arvoreAVL **eainicio,char chave){
    if (*eainicio == NULL){
        return 0;
    }else if ((*eainicio)->dado == chave){
        return 1;
    }else if (chave >(*eainicio)->dado){
        return procura(&((*eainicio)->dir),chave);
    }else if(chave <(*eainicio)->dado){
        return procura(&((*eainicio)->esq),chave);
    }
